...
public class WGraph_Algo implements weighted_graph_algorithms, Serializable {
    ...
    public boolean isConnected() {
        ...
        //First, we choose a node to start with.
        int iter = this.g.map.keySet().iterator().next();
        node_info startingNode = this.g.nodes.get(iter);
    ...
    public double shortestPathDist(int src, int dest) {
        if(!this.g.map.containsKey(src) || !this.g.map.containsKey(dest)){
            return -1;
        }
    ...
    public List<node_info> shortestPath(int src, int dest) {
        if(!this.g.map.containsKey(src) || !this.g.map.containsKey(dest)){ return null;}
        node_info start = g.getNode(src);
        node_info target = g.getNode(dest);
